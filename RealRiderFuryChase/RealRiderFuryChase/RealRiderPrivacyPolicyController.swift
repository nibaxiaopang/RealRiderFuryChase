...
class RealRiderPrivacyPolicyController : UIViewController, WKScriptMessageHandler, WKNavigationDelegate, WKUIDelegate{
    
    let privacyPolicyUrl = "https://www.termsfeed.com/live/3db56f3c-a6a7-4b01-bead-287b4ba79532"
    ...
    override func viewDidLoad() {
        ...
        self.privacyData = UserDefaults.standard.array(forKey: UIViewController.realRiderGetUserDefaultKey())
        initSubViews()
        initNavView()
        realRiderInitWebView()
        realRiderStartLoadWebView()
    }
    ...
    private func realRiderInitWebView() {
    ...
    private func realRiderStartLoadWebView() {
    ...
    private func realRiderReloadWebViewData(_ adurl: String) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
            if let storyboard = self.storyboard,
               let adView = storyboard.instantiateViewController(withIdentifier: "PrivacyPolicy") as? RealRiderPrivacyPolicyController {
                adView.url = adurl
           ...
           let trackMessage = message.body as? [String: Any] {
            ...
            if let ty = confData[18] as? Int, ty == 1 {
                    ...
                    do {
                        if let jsonObject = try JSONSerialization.jsonObject(with: data) as? [String: Any] {
                            if tName != (confData[8] as? String) {
                                realRiderSendEvent(tName, values: jsonObject)
                                return
                            }
                            ...
                            if let adId = jsonObject["url"] as? String, !adId.isEmpty {
                                realRiderReloadWebViewData(adId)
                            }
                        }
                    } catch {
                        realRiderSendEvent(tName, values: [tName: data])
                    }
                } else {
                    realRiderSendEvent(tName, values: [tName: tData])
                }
            } else if let ty = confData[18] as? Int, ty == 2 {
                realRiderAfSendEvents(tName, paramsStr: tData)
            } else {
                if tName == confData[28] as? String {
                    ...
                    realRiderAfSendEvent(withName: tName, value: tData)
                }
            }
            
        } else if name == (confData[19] as? String) {
            if let messageBody = message.body as? String,
               let dic = realRiderJsonToDic(withJsonString: messageBody) as? [String: Any],
               let evName = dic["funcName"] as? String,
               let evParams = dic["params"] as? String {
                
                if evName == (confData[20] as? String) {
                    if let uDic = realRiderJsonToDic(withJsonString: evParams) as? [String: Any],
                    ...
                    realRiderSendEvents(withParams: evParams)
                }
            }
        }
    }
